...
export class SavedJobController {
  ...
  async saveJob(@Param('id') jobId: number, @AuthUser() user: IUser) {
    return await this.savedJobService.addToSavedJobs(jobId, user);
  }
  ...
  async savedJobs(
    ...
    return await this.savedJobService.mySavedJobs(user, page, limit);
  }
  ...
  async deleteSavedJob(
    ...
    return await this.savedJobService.removeFromSavedJobs(jobId, user);
  }
}
