...
export class DashboardController {
  ...
  @HttpCode(201)
  async addSkill(@AuthUser() user: IUser, @Body() data: CreateSkillDto) {
    return await this.dashboardService.addSkills(user, data);
  }
  ...
  @HttpCode(201)
  async addJobCategory(@AuthUser() user: IUser, @Body() data: CreateJobCatDto) {
    return await this.dashboardService.addJobCat(data);
  }
  @HttpCode(201)
  @Post('/add-workType')
  ...
  async addWorkType(@AuthUser() user: IUser, @Body() data: CreateWorkTypeDto) {
    return await this.dashboardService.addWorkType(user, data);
  }
  @Delete('/skill')
  @ApiOperation({ summary: 'delete skill ' })
  @ApiQuery({ name: 'skillId', type: Number, description: 'Skill Id' })
  @ApiResponse({ status: 200, example: 'skill is deleted' })
  async deleteSkill(@Query('skillId', ParseIntPipe) skillId: number) {
    return await this.dashboardService.deleteSkill(skillId);
  }
  @Delete('/jobCategory')
  ...
  async deleteJobCategory(@Query('jobCatId', ParseIntPipe) jobCatId: number) {
    return await this.dashboardService.deleteJobCat(jobCatId);
  }
  @Delete('/workType')
  ...
  async deleteWorkType(@Query('workTypeId', ParseIntPipe) workTypeId: number) {
    return await this.dashboardService.deleteWorkType(workTypeId);
  }
  @Put('/jobCategory/:id')
  ...
  async updateJobCat(
    ...
    return await this.dashboardService.updateJobCat(data, workTypeId);
  }
  @Get('/data')
  @HttpCode(200)
  ...
  async getData(
    ...
    return await this.dashboardService.finds(page, limit, type);
  }
  @Get('/banned-users')
  @HttpCode(200)
  ...
  async getBannedUsers(
    ...
    return await this.userDashboard.BannedUsers(page, limit);
  }
  @ApiOperation({ summary: 'Ban a user by Id' })
  @ApiParam({ name: 'id', type: Number, description: 'User Id' })
  @Patch('ban-user/:id')
  @HttpCode(200)
  async BanUser(@Param('id',ParseIntPipe) userId: number) {
    return await this.userDashboard.BanUser(userId);
  }
  ...
  async unBanUser(@Param('id',ParseIntPipe) userId: number) {
    return await this.userDashboard.unBanUser(userId);
  }
  ...
  async changeRole(
    ...
    return await this.userDashboard.changeRole(userId, data);
  }
}
